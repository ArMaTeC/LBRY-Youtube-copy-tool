    ...
    Friend Class Program
        ...
        Shared Sub Form1debugout(ByVal fullstring As String)
            If (Form1.Form1Instance IsNot Nothing) AndAlso Not Form1.Form1Instance.IsDisposed Then
                ' Form1.Form1Instance.UpdateTextBox(fullstring)
        ...
        Public Shared Sub Scrape()
            ...
            Parallel.ForEach(CType(list, IEnumerable(Of String)), CType((Sub(currentUrl)
                                                                             ...
                                                                             Form1proxystringinfo(String.Format("Scraper | Total Scraped: {0} Duplicates removed: {1}", CObj(total), CObj((total - unique))))
                                                                             Console.WriteLine("Scraping from: " & currentUrl & " Done!")
                                                                             Application.DoEvents()
            ...
            Next
            Console.WriteLine("Done saving proxies to: " & str)
        ...
        Public Shared Sub Check()
            ...
            Console.WriteLine("Loaded: " & CObj(Variables.Proxies.Count) & " From your file!")
            Program.CheckProxies()
        End Sub

        Public Shared Sub CheckProxies()
            Program.ConvertProxies()
            Try
                ...
                Next
            Catch ex As Exception
                Console.WriteLine("Error CheckProxies: " + ex.ToString)
        ...
        Public Shared Sub Runner()
            Try
                ...
                While CUInt(Variables.ProxiesQueue.Count) > 0
                    Program.CheckProxy(Variables.ProxiesQueue.Dequeue())
                End While
            Catch ex As Exception
        ...
        Public Shared Sub CheckProxy(ByVal line As String)
            ...
            Try
                ...
                '               Dim request As HttpWebRequest = WebRequest.Create("http://i-dont-wanna.live/judge.php")
                Dim request As HttpWebRequest = WebRequest.Create("https://data.cityliferpg.com/lbry/ping.php")
                ...
                If Variables.Type.Contains("1") Then
                    ...
                    Console.WriteLine(String.Format("{0} | Working proxy HTTP/s", CObj(body)))



                    If body.Contains("Proxy detected") Then
                        Program.writer.FilePath = str
                        Program.writer.AppendToFile(String.Format("{0}", line))
                        Variables.Alive += 1
                        Console.WriteLine(String.Format("{0} | Working proxy HTTP/s", CObj(line)))
                        Form1proxystringinfo(String.Format("Checker Alive: {0} Dead: {1}", CObj(Variables.Alive), CObj(Variables.Dead)))
            ...
            Catch ex As Exception
                ...
                Form1proxystringinfo(String.Format("Checker Alive: {0} Dead: {1}", CObj(Variables.Alive), CObj(Variables.Dead)))
                Console.WriteLine(String.Format("{0} | Dead proxy", CObj(line)))
                Application.DoEvents()
            End Try
        End Sub
    End Class
